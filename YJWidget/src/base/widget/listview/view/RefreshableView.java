 ...
 * http://blog.csdn.net/sinyu890807/article/details/9255575
 这个类是整个下拉刷新功能中最重要的一个类，注释已经写得比较详细了，我再简单解释一下。
 首先在RefreshableView的构造函数中动态添加了刚刚定义的pull_to_refresh这个布局作为下拉头，
 然后在onLayout方法中将下拉头向上偏移出了屏幕，再给ListView注册了touch事件。
 之后每当手指在ListView上滑动时，onTouch方法就会执行。在onTouch方法中的第一行就调用了
 setIsAbleToPull方法来判断ListView是否滚动到了最顶部，只有滚动到了最顶部才会执行后面的代码，
 否则就视为正常的ListView滚动，不做任何处理。当ListView滚动到了最顶部时，如果手指还在向下拖动，
 就会改变下拉头的偏移值，让下拉头显示出来，下拉的距离设定为手指移动距离的1/2，
...
后面会演示使用的方法。
另外每次在下拉的时候都还会调用updateHeaderView方法来改变下拉头中的数据，比如箭头方向的旋转，
下拉文字描述的改变等。更加深入的理解请大家仔细去阅读RefreshableView中的代码。
 */
